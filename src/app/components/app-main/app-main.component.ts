  ...
  async mqttMessageParse(mqttContentModel: MqttContentModel): Promise<void> {
    ...
    switch (mqttContentModel.contentType) {
      case MqttContentType.None: {
        ...
        this.fDialogService.success(title, mqttContentModel.senderID, mqttContentModel.content);
        return;
      }
      ...
      case MqttContentType.USER_FILE_ADD: title = "mqtt-desc.title.user-file-add"; break;
    }
  ...
  async mqttConfirmFn(event: MouseEvent, message: any, router: Router): Promise<void> {
    ...
    router.navigateByUrl("/", { skipLocationChange: true }).then(async() => {
      switch (mqttContentModel.contentType) {
        ...
        case MqttContentType.USER_FILE_ADD: await router.navigate([`/${FConstants.USER_INFO_URL}/${mqttContentModel.targetItemPK}`]); break;
      }
    });
  }
